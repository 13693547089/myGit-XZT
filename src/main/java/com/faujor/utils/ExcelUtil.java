...
private void copyCell(HSSFCell srcCell, HSSFCell distCell) {
	 ...
	 * @return
	 */
    public static BigDecimal getDecimalCellValue(Row row,int cellIndex,FormulaEvaluator evaluator){
        ...
        switch(cellTypeEnum)
        {
            ...
            case FORMULA :
            {
            	...
            	Double iCellValue =cell.getNumericCellValue();
				value = new BigDecimal(iCellValue.intValue());
                break;
            }
            default:
				break;
    	...
    	try {
			...
			switch(cell.getCellTypeEnum()){
			    ...
			    case NUMERIC :
			    {
			    	...
			    	String sellStr=cellStyle.getDataFormatString();
			        if("m/d/yy".equals(sellStr)){  
			    ...
			    case FORMULA :
			    {
					evaluator=row.getSheet().getWorkbook().getCreationHelper().createFormulaEvaluator();
			      	CellValue tempCellValue = evaluator.evaluate(cell);
			      	value = BigDecimalUtil.getPercentage(new BigDecimal(tempCellValue.getNumberValue()), new BigDecimal(1)) ;
			        break;
			    }
			    case BOOLEAN :
			    {
     ...
     * @return
     */
    public static Date getDateCellValue(Row row,int cellIndex){
    	Date  value = null;
    	Cell cell=null;
    	if(row!=null){
    		cell=row.getCell(cellIndex);
        ...
        switch(cell.getCellTypeEnum())
        {
            case  NUMERIC :
            {
            	CellStyle cellStyle = cell.getCellStyle();
            	String sellStr=cellStyle.getDataFormatString();
                if("m/d/yy".equals(sellStr)||"yyyy/mm;@".equals(sellStr) || "yy/m/d".equals(sellStr)|| "dd-mmm-yy".equals(sellStr) || "yyyy/m/d".equals(sellStr)){  
     ...
     * @param pPosition 复制的位置
     */
    public static void copyRows(Sheet sheet,Sheet targetSheet, int startRow, int endRow, int pPosition) {
		int pStartRow = startRow - 1;
		int pEndRow = endRow - 1;
		int targetRowFrom;
		int targetRowTo;
		...
		for (i = pStartRow; i <= pEndRow; i++) {
			...
			columnCount = sourceRow.getLastCellNum();
			if (sourceRow != null) {
				Row newRow = targetSheet.createRow(pPosition - pStartRow + i);
				newRow.setHeight(sourceRow.getHeight());
				for (j = 0; j < columnCount; j++) {
					Cell templateCell = sourceRow.getCell(j);
					if (templateCell != null) {
