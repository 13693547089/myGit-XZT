...
layui.use([ 'form', 'table', 'laydate', 'layer', 'element' ,'upload'], function() {
	...
	var isChange = false;
	 ...
	 $("#exportBtn").click(function(){
		 var endDate=$('#planMonth').val()+"-1";
		 window.location.href="/actReach/exportBusyStock?endDate="+endDate
	 });	
	 $("#exportSuppProdBtn").click(function(){
//		 
		 var planMonth=$('#planMonth').val();
		 var planCode=$('#planCode').val();
		 window.location.href=prefix+"/exportTemp?planMonth="+planMonth+"&planCode="+planCode;
	 });	
	 $("#exportInvenBtn").click(function(){
//		 
		 var planMonth=$('#planMonth').val();
		 var planCode=$('#planCode').val();
		 window.location.href=prefix+"/exportInvenTemp?planMonth="+planMonth+"&planCode="+planCode;
	 });
	 $("#compareBtn").click(function(){
		 var planCode=$('#planCode').val();
		 window.location.href=prefix+"/export/compare?planCode="+planCode;
	 });
	 $("#importSuppProdBtn").click(function(){
		 var planCode=$('#planCode').val();
		 var perContent = layer.open({
				...
				area : [ '800px', '500px' ],
				maxmin : true,
				content : prefix + '/getCheckDataPage?planCode='+ planCode ,
			});
	...
	$('#innerCtrBtn').click(function(){
		var endDate=$('#planMonth').val()+"-1";
		window.location.href="/actReach/exportInnerControl?endDate="+endDate
	});
	$('#innerCtrWBtn').click(function(){
		var endDate=$('#planMonth').val()+"-1";
		window.location.href="/actReach/exportInnerControl?endDate="+endDate+"&wFlag=w"
	});
	if (type == 3) {
...
/*	//供应商等条件进行搜索过滤
	...
	$("#saveBtn").click(function() {
		...
		if(!flag){
			layer.msg("该月已经创建了备货计划，请勿重复创建！",{time:1000});
			return false;
		}
		if (planDetailArr.length > 0) {
			...
			if (type == "1") {// 新建保存
				...
				var remark = "备货计划审核: " + planDesc;
				flag = taskProcess(mainId, "invenPlanAudit", remark, "save");
			} else {// 编辑保存
				flag = "init_success";
			}
	...
	$("#submitBtn").click(function() {
		...
		if (rows.length > 0) {
			...
			var flag2 = taskProcess(mainId, "invenPlanAudit", remark, "process");
...
//				window.history.go(-1);
				tuoBack("invenPlanEdit","#searchBtn");
			},
			error : function() {
				layer.msg("操作失败！", {
					time : 1000
...
*/		var options = {
	...
	function addFontBold(row,col){
		debugger;
		 var mateDesc= row.mateDesc;
		 var col= row.col;
		 if(mateDesc=='小计' && col!=null && col!=''){
			 return '<span class="bold">'+ col +'</span>';
		 }else{
	...
	//监听表格事件  分解或者撤销	
	 table.on('tool(mateTable)', function(obj) {
			...
			if(planCode==null || planCode==''){
				...
				return ;
			}else if(mainStatus =='已提交' && obj.event =='cancle'){
				layer.msg('备货计划已提交不能撤销',{time:1000});
				return ;
			}else if(mateDesc=='小计' || mateDesc=='合计'){
				layer.msg(mateDesc+"物料无法操作!",{time:1000});
				return;
			}
	 ...
	 table.on('edit(mateTable)', function(obj){ //注：edit是固定事件名，test是table原始容器的属性 lay-filter="对应的值"
		 if(obj.field == 'deliveryPlan'){
			 var newMates=[];
			 for(var i=0;i<planDetailArr.length;i++){
				 var mate=planDetailArr[i];
				 if(mate.mateDesc!='小计' && mate.mateDesc!='合计'){
					 var beginStock=mate
					 newMates.push(mate);
				 }
			 }
			 ...
			 var status=data.status;
			 if(status=="已分解"){
				 planDetailArr=calSubTotal(newMates);
				 initMateTable(planDetailArr);
			 }else{
				 var data=obj.data; 
				 var deliveryPlan=data.deliveryPlan;
				 debugger;
				 if(isNaN(deliveryPlan) || deliveryPlan<=0){
					 data.deliveryPlan =0;
				 }
			 }
...
/*	 table.on('tool(mateTable)', function(obj){ 
		 var data = obj.data; //获得当前行数据
		if(obj.event == 'editTable'){
			 obj.update({
				 deliveryPlan:data.deliveryPlan,prodPlan:2136162
			 });
			 console.log(data.deliveryPlan);
		 }
		});	*/ 
	   /**
		 * 分解备货计划详情
		...
		function decomposeDetail(data){
			...
			var perContent=layer.open({
				  ...
				  area: ['800px', '500px'],
				  maxmin: true,
				  content: prefix+'/getDecomposePage?mateCode='+data.mateCode+"&mateDesc="+data.mateDesc+"&deliveryPlan="+data.deliveryPlan+"&mainId="+mainId+"&planMonth="+planMonth+"&nextMonthDeliveryNum="+nextMonthDeliveryNum+"&planDetailId="+data.id+"&status="+data.status,
