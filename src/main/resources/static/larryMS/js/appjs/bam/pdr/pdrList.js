...
layui.use(['table','laydate'], function() {
	...
	initListTable(prefix+"/getPdrPageList");
	//初始化表格
	...
	//表格监听事件
	 table.on('tool(docTool)', function(obj) {
		...
		if(obj.event == 'show'){
			//查看
			var type="2";
			var url=prefix+"/pdrDetailPage?mainId="+id+"&type="+type;
	...
	function reloadTable(){
		...
		var url=prefix+"/getPdrPageList?search_pdrCode="+pdrCode+"&search_status="+statusName
	...
	$("#createBtn").click(function(){
		var type='1';
		var url=prefix+"/pdrDetailPage?mainId="+"&type="+type;
		//window.location.href=url;
		tuoGo(url,'pdrDetail','list-table');
	});
