...
layui.use([ 'table', 'laydate','form'], function() {
	...
	$("#goBack").click(function() {
		...
		tuoBack('.ContactSheetDetail','#search');
	}); 
	// 年月选择器
	laydate.render({
		elem : '#cutMonth',
	...
	/*sumOutNum : item.sumOutNum,
	sumInveNum : item.sumInveNum,
	sumProdNum : item.sumProdNum,*/
	...
	//引用包材打切联络单号触发
	/*form.on("select(citeLiaiCode)",function(obj){
	    if(citeLiaiCode == '' || citeLiaiCode == null || citeLiaiCode == undefined){
	    	layer.msg("请选择引用单号");
	    	return;
	    }
	    var cutMonth = $("#cutMonth").val();
	    if(cutMonth == '' || cutMonth == null || cutMonth == undefined){
	    	layer.msg("请填写打切月份");
	    	return;
	    }
	   ...
	   var citeLiaiCode =obj.value;
	   tableData=[];
		// 获取引用后的物料数据
		$.ajax({
			type : "post",
			url : "/citeBaoCaiCutLiaiMateData",
			data:{
				citeLiaiCode:citeLiaiCode,
				headerFiled:str,
				suppId:suppId,
				cutMonth:cutMonth
			}
