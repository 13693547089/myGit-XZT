...
layui.use([ 'table','upload','layer','form','laydate'], function() {
	...
	if(type=='1'){
		if(userType=='supplier'){
			processCode = 'quoteAudit';
		}else{
			processCode = 'UserQuoteAudit';
		}
	}
	if(type==3 || type==4){
		disableFormItem();
...
//				mateList.splice(index,1);
	...
	$("#addMateBtn").click(function(){
		var row={id:new Date().getTime(),priceList:[],startDate:new Date().Format("yyyy-MM-dd"),endDate:'9999-12-31'};
...
//		newData.push(totalData);
	 ...
	 * 附件表格的工具条点击事件
	 */
	table.on('tool(attr-table-filter)', function(obj){ 
		  var mydata = obj.data; 
		  var layEvent = obj.event; 
		  if(layEvent === 'downLoadAttr'){ //下载
	...
	function initQuoteAttrTable(data){
		...
		var status=$("#status").val();
		if( type==4 || userType=="user" && status=='已保存'&&type!=3 || userType=="user" && status=='已退回'&&type!=3 || userType=="user" && status==undefined || userType=="user" && status==""){
