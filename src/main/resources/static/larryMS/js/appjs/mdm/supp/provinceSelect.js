...
function BindProvinceProv(prov){  
	//debugger;
	var opt0  
	if(prov!=''){
		opt0=prov
...
var ddlProvince = document.getElementById("ddlProvince");  
ddlProvince.innerHTML = "";  
ddlProvince.options[0] = new Option(opt0,opt0);  
for(var i=0; i<ProvinceCount-1; i++){
	if(china[i][0]==opt0){
		i++;
		ddlProvince.options[i+1] = new Option(china[i][0],china[i][0]);  
	}else{
...
function selectMoreCitymo(sbj,city){
	//debugger;
	//console.log(sbj);
	var opt0;
	if(city!=''){
    ...
    if(sbj.selectedIndex==0){  
        ...
        return;  
            ...
            var ddlCity = document.getElementById("ddlCity");  
            ddlCity.innerHTML = "";  
            ddlCity.options[0] = new Option(opt0,opt0);  
