...
layui.define([ 'jquery', 'layer', 'laytpl', 'element' ],function(exports) {
	var $ = layui.$, layer = layui.layer, laytpl = layui.laytpl, device = layui
			.device(), element = layui.element, doc = document, online = false, 
			conf = {
				modules : {}
				// 记录自定义模块物理路径
			}, MOD_NAME = 'larryms',
			// Message模板
			LARRY_INFO = [
				'<div class="larryms-message" id="messageBox">',
				'<div class="larryms-message-box clearfix">',
				'<i class="larry-icon {{d.ICONS}}"></i>',
				'<div class="resultmsg">{{d.MSG}}</div>', '</div>',
				'</div>' ].join(''),
	...
	// larryms构造器
	larryms = function() {
		this.version = 'LarryMS 2.07 Beta2', this.tit = [
				'LarryMS提示您', 'larryMS错误提示', 'larryMS参数配置错误提示',
				'数据源配置错误', '关闭失败提示', '操作成功', '操作失败',
				'LarryMS Ajax 调试信息控制台' ], this.config = {
			plusDir : '/common/plus/',
			jqDefined : undefined,
			jqUrl : undefined
	...
	function removeLink(id) {
		var head = document.getElementsByTagName('head')[0], links = document.getElementById(id);
		if (links !== null) {
	...
	//						layui.data('larry_menu', null);
		...
		if (device.android || device.ios
			...
			var mMenu = $('#larrymsMobileMenu'), mShade = $('#larrymsMobileShade');
			mMenu.on('click', function() {
				if ($('#larry_layout').hasClass(
						'larryms-mobile-layout')) {
					$('#larry_layout').removeClass(
							'larryms-mobile-layout');
				} else {
					$('#larry_layout').addClass(
							'larryms-mobile-layout');
				}
			});
		...
		var placeholder = '';
		$(
				"input.larry-input[type='text'],input.larry-input[type='password']")
				.on('focus', function() {
	...
	larryms.prototype.font = function(icon, url, online) {
		if (icon == 'larry-icon' || icon == 'font-awesome') {
			if ($('.larry-icon').length > 0
					|| $('.fa').length > 0) {
				this
						.alert('默认支持的字体库已完成自动加载，或请检查是否修改了默认目录结构未成功加载icon样式，对于默认icon字体请勿使用该方法加载');
				return false;
			}
		}
	...
	larryms.prototype.fontset = function(configs) {
		...
		that.font(that.fonts.icon, that.fonts.url,
				that.fonts.online);
	}
	...
	larryms.prototype.message = function(msg, mark, icon, time) {
		var that = this, msg = msg || 'default', mark = mark
				|| 'other', icons = icon || 'larry-xiaolian1', flag = 0, Time;
		...
		function dialog() {
			return this.index = parent.layer
					.open({
						type : 1,
						closeBtn : flag,
						anim : Math.ceil(Math.random() * 6),
	 ...
	 * @return {[type]}
	 */
	larryms.prototype.plugin = function(jsName, callback,
			jsConf) {
		if ($.isPlainObject(jsConf)) {
			...
			this.set(jsConf);
		} else if (!$.isPlainObject(jsConf)
				&& jsConf !== undefined) {
			this
					.error(
							'第三方jQuery插件路径参数配置错误，请书写正确配置格式，否则将从默认路径加载插件！',
							this.tit[2]);
		} else {
			// 若未配置将设置默认第三方插件存放目录
			this.set({
		...
		if (typeof jsName == 'string') {
			var jsPath = this.config.plusDir + jsName, plus = doc
					.createElement('script');
			plus.type = 'text/javascript';
			plus.src = jsPath;
			plus.async = false;
			// 1、获取当前执行js文件名称及位置
			var name = getPath().names, obj = getPath(name).obj[getPath(name).index], body = doc
			...
			if (!this.config.jqDefined) {
				if (!window.jQuery && $) {
					...
					this
							.error(
									'上下文环境中未检测jQuery对象，请正确配置自定义jq插件或手动在页面中引入 否则任何依赖jquery的第三方插件将不能正常运行！！！',
									this.tit[1]);
				}
				runHack();
			} else {
				// 自定义了jq版本
				var jqPath = this.config.jqUrl
						+ this.config.jqDefined, head = doc
	 ...
	 * @param {[thatObj]}
	 */
	larryms.prototype.ajaxDebug = function(XMLHttpRequest,
			textStatus, errorThrown, thatObj) {
		if (this.debug) {
	...
	// this.close();
	// return this.index =
	// layer.alert(msg,{end:callback,scrollbar:false});
	// }else{
	// return true;
	// }
	// };
	...
	larryms.prototype.logOut = function(url, ok, no) {
		layer.confirm('确定退出系统吗！', function(url) {
			$.ajax({
				url : "/redisLogout",
				type : "get",
				async : false,
